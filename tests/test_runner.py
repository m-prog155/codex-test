...
def test_run_frame_rescue_probe_accepts_structured_text_with_alpha_count_gate() -> None:
    config = make_config()
    config.ocr.probe.enabled = True
    config.ocr.probe.rescue_min_confidence = 0.99
    config.ocr.rescue_probe.enabled = True
    ...
    config.ocr.rescue_probe.rescue_reject_repeated_required_char = True
...
def test_run_frame_rescue_probe_rejects_text_when_alpha_count_does_not_match() -> None:
    config = make_config()
    config.ocr.probe.enabled = True
    config.ocr.probe.rescue_min_confidence = 0.99
    config.ocr.rescue_probe.enabled = True
    ...
    assert result.matches[0].recognition is None
    assert result.matches[0].diagnostic is not None
    assert result.matches[0].diagnostic.notes == ["rescue_probe_rejected:皖AZ0037"]


def test_run_frame_rescue_probe_rejects_text_when_required_char_repeats() -> None:
    config = make_config()
    config.ocr.probe.enabled = True
    config.ocr.probe.rescue_min_confidence = 0.99
    config.ocr.rescue_probe.enabled = True
